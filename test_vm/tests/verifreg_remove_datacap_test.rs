...
fn remove_datacap_simple_successful_path() {
    ...
    add_verifier_params_ser =
        serialize(&add_verifier_params.clone(), "add verifier params").unwrap();
    apply_ok(
        &v,
        VERIFREG_ROOT_KEY_ADDR,
        *VERIFIED_REGISTRY_ACTOR_ADDR,
    ...
    let add_verified_client_params_ser =
        serialize(&add_verified_client_params.clone(), "add verifier params").unwrap();
    apply_ok(
        &v,
        verifier1,
        *VERIFIED_REGISTRY_ACTOR_ADDR,
    ...
    let mut verifier1_payload = SIGNATURE_DOMAIN_SEPARATION_REMOVE_DATA_CAP.as_bytes().to_vec();
    ...
    let mut verifier2_payload = SIGNATURE_DOMAIN_SEPARATION_REMOVE_DATA_CAP.as_bytes().to_vec();
    ...
    assert_eq!(allowance_to_remove.clone(), remove_datacap_ret.data_cap_removed);
    ...
    assert_eq!(*data_cap, verifier_allowance.clone().sub(allowance_to_remove.clone()));
    ...
    verifier1_payload = SIGNATURE_DOMAIN_SEPARATION_REMOVE_DATA_CAP.as_bytes().to_vec();
    verifier1_payload.append(&mut verifier1_proposal_ser);
    ...
    verifier2_payload = SIGNATURE_DOMAIN_SEPARATION_REMOVE_DATA_CAP.as_bytes().to_vec();
    verifier2_payload.append(&mut verifier2_proposal_ser);
